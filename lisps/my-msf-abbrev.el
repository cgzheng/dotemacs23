;;; my-msf-abbrev.el --- maintain abbrevs in a directory tree
...
(define-key fld-keymap (kbd "<backtab>") 'fld-prev)
...
;; 		(cursor-in-echo-area t)
		...
		(move-overlay o
				 ...
				 (cadr (cdr (assoc choice-index
						  choice-info)))))))
         ;; RET或m或M
	     ((or (equal keys [?\C-m]) (equal keys [?m]) (equal keys [?M]))
	      (setq out (nth choice-index choices-list)
		    done t))
         ;; C-g或q或Q
	     ((or (equal keys [?\C-g]) (equal keys [?q]) (equal keys [?Q]))
	      (setq done t))
	     (t nil))))))
    (message "")
    out))
...
(defun msf-abbrev-directory-files (dir)
  (when (not (equal (file-name-nondirectory dir) "semantic.cache"))
  (delq nil
	(mapcar
	 (lambda (x)
